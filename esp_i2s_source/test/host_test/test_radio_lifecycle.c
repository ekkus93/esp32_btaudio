...
/* ---- Tests defined in test_radio_lifecycle_faults.c ----
 * RH-S3-16 (NVS error propagation), RH-S3-02 (partial worker exit
 * timeout/fault), 7.11 (decoder-only failure, event group failure, ASan
 * verification). Split out to keep both files under the line-count
 * guideline; RUN_TEST() below still exercises them from this binary's
 * single main(). */
