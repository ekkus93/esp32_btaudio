...
2. I2S master transmitter outputs tone to GPIO18/19/21 (via ALSA)
3. Continuous playback for 5 minutes without dropouts

Usage:
    # On Raspberry Pi with I2S configured:
...
Hardware Setup:
    - Raspberry Pi with I2S enabled (dtoverlay=i2s-mmap in /boot/config.txt)
    - Connect ESP32 esp_bt_audio_source I2S input:
        - GPIO18 (BCM) → ESP32 I2S BCLK
        - GPIO19 (BCM) → ESP32 I2S WS
        - GPIO21 (BCM) → ESP32 I2S DIN
        - GND → GND
...
class Milestone1Test:
    ...
    Coordinates AudioEngine (tone generation) and I2SDriver (I2S transmission)
    ...
    def start(self):
        ...
        print("=" * 70)
        print("Milestone 1: Basic I2S Tone Generation Test")
        print("=" * 70)
        print()
        print("Configuration:")
        ...
        print()
        print("I2S GPIO Pins (BCM numbering):")
        print(f"  BCLK: GPIO{self.config.get('i2s.gpio_bclk', 18)}")
        print(f"  WS:   GPIO{self.config.get('i2s.gpio_ws', 19)}")
        print(f"  DOUT: GPIO{self.config.get('i2s.gpio_dout', 21)}")
        print()
        ...
        try:
            ...
            # Start I2S driver (transmits samples via ALSA I2S)
            print("[2/3] Starting I2S driver...")
            self.i2s_driver.start()
            print("      ✓ I2S driver running (ALSA transmission)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Milestone 1: Basic I2S Tone Generation Test",
