...
2026-01-01 23:40:19 Deduplicated bounded mem helpers: bt_app_core now includes mem_util and drops local safe_memset/safe_memcpy; bt_connection_manager switches to util_parse_mac/util_format_mac and removes local hex/parser helpers; verified bt_streaming_manager safe_mem calls use shared signatures. Tests: `pytest -q` from repo root passes (PytestExperimentalApiWarning only). Build: `idf.py -C esp_bt_audio_source build` succeeds after ESP-IDF export.
