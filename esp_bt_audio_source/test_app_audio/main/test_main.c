...
static void maybe_run_group_override(void)
{
#ifdef UNITY_AUDIO_TEST_GROUP_OVERRIDE
    const char *group = UNITY_AUDIO_TEST_GROUP_OVERRIDE;
    if (group != NULL && group[0] != '\0') {
        ESP_LOGI(TAG, "UNITY compile-time override for group '%s'", group);
        run_test_group(group);
        ESP_LOGI(TAG, "UNITY override '%s' completed", group);
        printf("--- SUMMARY ---\n");
        printf("----- UNITY TEST COMPLETE: %s -----\n", Unity.TestFailures ? "FAIL" : "PASS");
