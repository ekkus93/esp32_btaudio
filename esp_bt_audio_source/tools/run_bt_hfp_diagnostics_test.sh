...
build_dir="${test_dir}/build_host_tests/bt_hfp_diagnostics"
binary="${build_dir}/test_bt_hfp_diagnostics"
...

{
    "${CC:-cc}" "${common_flags[@]}" \
        "${unity_dir}/unity.c" \
        "${project_dir}/components/bt_manager/bt_hfp_manager_fd13.c" \
        "${test_dir}/test_bt_hfp_diagnostics.c" \
        -o "${binary}"
} 2>&1 | tee "${compile_log}"
